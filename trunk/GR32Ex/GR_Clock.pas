 ...
 * Purpose  : the Clock Component
Component Overview

  The TGRClock component displays an analog clock on a form. The clock can be used
  as a real-time clock or it can be used as a static time display. When used as
  a real-time clock, the clock displays the system's current time, updated every
  second. When used as a static time display, the clock displays a time as
  determined by the Time property. A static time display can be used to display
  schedules, meeting times, and so on. 

  Both the clock face and the clock hands are customizable. The clock face is
  determined by the ClockFace property. If ClockFace is not assigned, the clock
  will be drawn with a default clock face. To display a picture on a clock face,
  assign a picture to the ClockFace property. The bitmap supplied will be used as
  the clock face with the hands drawn on top of the clock face. Orpheus comes with
  several clock faces that you can use in your applications. These bitmaps can be
  found in the Pics directory. The clock hands can be customized to modify the
  hand length, color, and fill style. The second hand can be shown or hidden as desired.

 * Required : 
   Now I decide port this to my GRControls. See GR_Clock.pas
 * Author   : Riceball <teditor@mailroom.com>
 * Copyright: 2001 by Riceball. All rights reserved.
 * License  : This project is subject to the Mozilla Public License Version 1.1 
              (see the "License.txt"); you may not use this project except in 
...
uses
  ...
  QGraphics;
  {$ENDIF}
  //Controls, Dialogs, Forms, Menus, Messages;
...
Procedure Register;
Begin
  RegisterComponents('Additional', [TGRClock]);
End;
...
begin
  ...
  begin
    ...
    SetTime(MsToDateTime(FTimer.ElapsedTime));
end;
...
var
  aDate : TDateTime;
begin
  aDate := MsToDateTime(FTimer.ElapsedTime);
  Result := Trunc(aDate);
end;
...
var
  ...
  aTime : TDateTime;
begin
  aTime := MsToDateTime(FTimer.ElapsedTime);
...
begin
  ...
  Result := FTimer.ElapsedTime div 1000;
end;
...
var
  ...
  procedure DrawMarks(const aClockMark: TGRCustomClockMark; Step: integer);
  var
    i: Integer;
    MarkX     : Integer;
    MarkY     : Integer;
...
begin
  ...
  begin
    ...
    begin
      ...
      DrawMarks(FMinuteMarks, 1);
      DrawMarks(FHourMarks, 5);
    end;
  end;
