 ...
 * License.
 *
 * The Original Code is GR_StdCtrls
 *
...
uses
  ...
  private
    ...
    procedure DrawButtonGlyph(bmp: TBitmap32; const GlyphPos: TPoint; State:
      TGRButtonState; Transparent: Boolean);
    procedure DrawButtonText(bmp: TBitmap32; const aFont: TFont32; const
      Caption: string; TextBounds: TRect; State: TGRButtonState; BiDiFlags:
      Longint);
    procedure GlyphChanged(Sender: TObject);
...
procedure TGRCustomButton.WMLButtonUp(var Message: TWMLButtonUp);
var
  DoClick: Boolean;
begin
...
begin
  ...
  FAniTimer := TSimpleTimer.CreateEx(cTimerInterval, DoAniTimer);
...
begin
  ...
  begin
    TextBounds := Rect(0, 0, Client.Right - Client.Left, 0);
    DrawText(bmp.Handle, PChar(aCaption), Length(aCaption), TextBounds,
      DT_CALCRECT or BiDiFlags);
    TextSize := Point(TextBounds.Right - TextBounds.Left, TextBounds.Bottom -
      TextBounds.Top);
  end
  ...
  {$ifdef Debug}
  {//with aBitmap32.ClipRect do
    SendDebug('GlyphPos:'+ IntToStr(GlyphPos.X)+ ';'+IntTOstr(GlyphPos.Y));
    SendDebug('Client:'+ IntToStr(Client.Left)+ ';'+IntTOstr(Client.Top));
    with Offset do
      SendDebug('OffsetPos:'+ IntToStr(X)+ ';'+IntTOstr(Y));
    with TextPos do
      SendDebug('TextPos:'+ IntToStr(X)+ ';'+IntTOstr(Y));
  //}
  {$Endif}
end;
...
procedure TGRButton.WMLButtonDown(var Message: TWMLButtonDown);
begin
  inherited;
  {
  if not Down and not FGlyph.Empty and (FAnimating <> cAniRunning) then
...
procedure TGRButton.WMLButtonUp(var Message: TWMLButtonUp);
begin
  inherited;
  {
  if not FGlyph.Empty and (FAnimating <> cAniRunning) then
  begin
    if not Down then
      //Init the time count to zoom out(放大).
      AniStart
    else if FAniCurTime > 0 then //coutinue zoom in(缩小)
      FAnimating := cAniRunning;
  end;
  //}
end;
