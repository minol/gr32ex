...
{                                                                             }
{     This is an enhanced version of the BZip2 unit by Edison Mera            }
{     Modifications were needed for SFDS compatibility                        }
{                                                                             }
{  revision history                                                           }
{    2006.08.17  Modified this unit to include the enhancements applied in    }
{                  the ZlibEx unit:                                           }
{                  - Enhanced TBZDecompressionStream.Read to adjust source    }
{                     stream position upon end of compression data            }
{                  - Fixed endless loop in TBZDecompressionStream.Read when   }
{                     destination count was greater than uncompressed data    }
{                  - Added soFromEnd condition to TBZDecompressionStream.Seek }
{                Renamed unit from BZip2 to BZip2Ex                           }
{    2007.06.04  Modified TBZDecompressionStream to allow 64-bit seeking mode.}
{                Renamed unit from BZip2Ex to sfds_BZip2Ex                    }
{                                                                             }
{                                                                             }
{*****************************************************************************}
...
const
SBZip2Invalid = 'Invalid bzip2 stream operation';
...
begin
  raise EBZCompressionError.Create(SBZip2Invalid);
end;
...
begin
  ...
  while (FBZRec.avail_out > 0) and (bzresult <> BZ_STREAM_END) do
  begin
    ...
    bzresult := CCheck(BZ2_bzDecompress(FBZRec));
  end;

  {Start of modification to be like in the ZlibEx unit}
  {Modification is needed for the SFDS to function correctly, and does not modify
  the bzip2 algorithm in any way}
  if (bzresult = BZ_STREAM_END) and (FBZRec.avail_in > 0) then
  begin
    FStrm.Position := FStrm.Position - FBZRec.avail_in;
    FStrmPos := FStrm.Position;
...
begin
  raise EBZDecompressionError.Create(SBZip2Invalid);
end;
...
var
  ...
  {$ENDIF}
begin
  if (Offset = 0) and (Origin = {$IFNDEF VERSION_5_OR_BELLOW}soBeginning{$ELSE}soFromBeginning{$ENDIF}) then
  begin
  ...
  else if ((Offset >= 0) and (Origin = {$IFNDEF VERSION_5_OR_BELLOW}soCurrent{$ELSE}soFromCurrent{$ENDIF})) or
    (((Offset - FBZRec.total_out_lo32) > 0) and (Origin = {$IFNDEF VERSION_5_OR_BELLOW}soBeginning{$ELSE}soFromBeginning{$ENDIF})) then
  begin
    {$IFNDEF VERSION_5_OR_BELLOW}  
    TmpOffset := Offset;
    {$ENDIF}
    if Origin = {$IFNDEF VERSION_5_OR_BELLOW}soBeginning{$ELSE}soFromBeginning{$ENDIF} then {$IFNDEF VERSION_5_OR_BELLOW}Dec(TmpOffset,FBZRec.total_out_lo32);{$ELSE}Dec(offset,FBZRec.total_out_lo32);{$ENDIF}
    if {$IFNDEF VERSION_5_OR_BELLOW}TmpOffset{$ELSE}offset{$ENDIF} > 0 then
    begin
      for I := 1 to {$IFNDEF VERSION_5_OR_BELLOW}TmpOffset{$ELSE}offset{$ENDIF} div sizeof(Buf) do
        ReadBuffer(Buf, sizeof(Buf));
      ReadBuffer(Buf, {$IFNDEF VERSION_5_OR_BELLOW}TmpOffset{$ELSE}offset{$ENDIF} mod sizeof(Buf));
    end;
  end
  {Start of modification to be like in the ZlibEx unit}
  {Modification is needed for the SFDS to function correctly, and does not modify
  the bzip2 algorithm in any way}
  else if (offset = 0) and (origin = {$IFNDEF VERSION_5_OR_BELLOW}soEnd{$ELSE}soFromEnd{$ENDIF}) then
  begin
