...
begin
  lbLog.Items.Clear;
  if not TestGSSCreation then Exit;
  if not TestGSSFlatFileSystem then Exit;
  if not TestGSSBigAndSmall then Exit;
  if not TestGSSFragmentedFiles then Exit;
  if not TestGSSFolders then Exit;
  if not TestGSSTruncation then Exit;
  if not TestGSSExists then Exit;
  if not TestGSSEnumerating then Exit;
  if not TestGSSMovingAndDeleting then Exit;
  if not TestGSSAttributes then Exit;
  if not TestGSSCompacting then Exit;
  if not TestGSSExceptions then Exit;
  Log('*** OK ***');
...
begin
  if TestGSSBigAndSmall then
    Log('*** OK ***');
end; { TForm1.btnTestBigAndSmallClick }
...
begin
  ...
  try
    ...
    TestFile(storage, '/secondfolder/secondsubfolder/firstfile.dat', true, -1);
    TestFile(storage, '/secondfolder/secondsubfolder/secondfile.dat', true, -1);
    if not Expect(storage.FileExists('/firstfile.dat'), true) then Exit;
    if not Expect(storage.FileExists('/secondfile.dat'), true) then Exit;
    if not Expect(storage.FileExists('/thirdfile.dat'), false) then Exit;
    if not Expect(storage.FileExists('/firstfolder/firstfile.dat'), true) then Exit;
    if not Expect(storage.FileExists('/firstfolder/secondfile.dat'), true) then Exit;
    if not Expect(storage.FileExists('/firstfolder/thirdfile.dat'), false) then Exit;
    if not Expect(storage.FileExists('/secondfolder/firstfile.dat'), false) then Exit;
    if not Expect(storage.FileExists('/secondfolder/secondsubfolder/firstfile.dat'), true) then Exit;
    if not Expect(storage.FileExists('/secondfolder/secondsubfolder/firstfile.dat'), true) then Exit;
    if not Expect(storage.FileExists('/'), false) then Exit;
    if not Expect(storage.FileExists('/firstfolder'), false) then Exit;
    if not Expect(storage.FileExists('/firstfolder/'), false) then Exit;
    if not Expect(storage.FileExists('/secondfolder/firstsubfolder'), false) then Exit;
    if not Expect(storage.FileExists('/secondfolder/firstsubfolder/'), false) then Exit;
