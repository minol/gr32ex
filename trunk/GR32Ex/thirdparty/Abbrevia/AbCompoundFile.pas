...
{Summary to store multiple, compressed files as part of a single file. }
{
This functionality emulates the functionality of Microsoft's Structured Storage mechanism, but does not rely on COM interfaces to achieve this functionality. Abbrevia's compound file class encapsulates a miniature file system within the single compound file. The mechanism employed to achieve this end result is similar to the FAT file system. 

There are three fundamental sections of Abbrevia's TAbCompoundFile structure that are maintained internally: the System block, the Root Directory block, and the File Allocation Table (FAT). Abbrevia's TAbCompoundFile class manages each of these sections and insulates the user from the intricacies of dealing with each section. Despite the fact that the user needs to know nothing of these structures, some background information may be helpful. 
  ...
  License:
    ...
    * The Original Code is $RCSfile: AbCompoundFile.pas,v $.
    * The Initial Developers of the Original Code are TurboPower Abbrevia
    * Portions created by Turbo Power is Copyright (C) 1997-2002
    * All rights reserved.
...
begin
  ...
  FStream.Read(Sig[0], sbSignatureSize);
  if Sig <> 'AbCompoundFile' then begin
