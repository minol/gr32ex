...
{$IFDEF PNG_Supports}
  procedure AssignFromPNG(aPNG: TPNGObject);
  type
  ...
  begin
    ...
    try
      LBMP.Assign(aPNG);
      AssignFromBitmap(LBMP);
    finally
      LBMP.Free;
    end;
    if Empty then exit;
    case aPNG.TransparencyMode of
      ptmBit:
        begin
        	//Clear;exit;
          TransparentColor := Color32(aPNG.TransparentColor) and $00FFFFFF;
          DstP := @Bits[0];
          ...
          if FDrawMode = dmOpaque then FDrawMode := dmBlend;
        end;
      ptmPartial:
        begin
        	//Clear;exit;
          if (aPNG.Header.ColorType = COLOR_GRAYSCALEALPHA) or
            (aPNG.Header.ColorType = COLOR_RGBALPHA) then
          begin
            DstP := @Bits[0];
            for y := 0 to Height - 1 do
            begin
              p := aPNG.AlphaScanline[y];
              for x := 0 to Width - 1 do
              begin
                PColor32Rec(DstP)^.rgbAlpha := p[x];
                inc(DstP);
              end;
            end;
          end;
...
{$IFDEF PNG_Supports}
    ...
    begin
    	AssignFromPNG(TPNGObject(Source));
    	Exit;
    end
{$ENDIF}
    else if Source is TGraphic then
...
{$IFDEF PNG_Supports}
        else if TPicture(Source).Graphic is TPNGObject then
          AssignFromPNG(TPNGObject(TPicture(Source).Graphic))
{$ENDIF}
