unit GR_FastFX;                            // FastFX
// Updated: 2006-1-16
//Modified by riceball to GR32
...
procedure Invert(Bmp: TBitmap32);
...
{
procedure Shift(Bmp: TBitmap32; xAmount, yAmount: Integer); //bpp > 4
procedure AvgFrames(Dst: TBitmap32; Src: array of TBitmap32; Count: Integer);
procedure Flip(Bmp: TBitmap32);
procedure Flop(Bmp: TBitmap32);
}
...
(*
procedure SquareWave(Src, Dst: TBitmap32; X, Y, z: Double); //bpp > 4
procedure RoundWave(Src, Dst: TBitmap32; X, Y, z: Double); //bpp > 4


{------------------------------------------------------------------------------}
procedure CopyRectToDIB(Dst, Src: TBitmap32; xSrc, ySrc, wSrc, hSrc: Integer);
procedure CopyDIBToRect(Dst, Src: TBitmap32; xDst, yDst, wDst, hDst: Integer);
{------------------------------------------------------------------------------}
//*)
...
begin
  if not Bmp.Empty then
    InvertMem(Bmp.Bits, Bmp.Width * Bmp.Height)
end;
...
(*
procedure Shift(Bmp: TBitmap32; xAmount, yAmount: Integer);
var
  Y, Size: Integer;
  Line, Buf: PLine8;
begin
  ...
  begin
    if yAmount < 0 then
      yAmount := Bmp.Height - (-yAmount mod Bmp.Height);
    if yAmount > Bmp.Height then
      yAmount := yAmount mod Bmp.Height;
    if Bmp.Size <> 0 then
    begin
      Size := yAmount * Bmp.BWidth;
      GetMem(Buf, Size);
      Move(Bmp.Bits^, Buf^, Size);
      Move(Bmp.Scanlines[yAmount]^, Bmp.Bits^, (Bmp.Height - yAmount) * Bmp.BWidth);
      Move(Buf^, Bmp.Scanlines[Bmp.Height - yAmount]^, Size);
      FreeMem(Buf);
    end
    else
    begin
      ...
      for Y := yAmount to Bmp.Height - 1 do
        Move(Bmp.Scanlines[Y]^, Bmp.Scanlines[Integer(Y) - yAmount]^, Size);
      Line := Pointer(Buf);
      for Y := Bmp.Height - yAmount to Bmp.Height - 1 do
      begin
  ...
  begin
    if xAmount < 0 then
      xAmount := Bmp.Width - (-xAmount mod Bmp.Width);
    if xAmount > Bmp.Width then
      xAmount := xAmount mod Bmp.Width;
...
procedure AvgFrames8(Dst: TBitmap32; Src: array of TBitmap32; Count: Integer);
var
  Lut: TLut;
  I, X, Y: Integer;
  pb, pc: PByte;
...
procedure AvgFrames(Dst: TBitmap32; Src: array of TBitmap32; Count: Integer);
begin
    AvgFrames16(Dst, Src, Count);
end;

procedure Flip32(Bmp: TBitmap32);
var
  X, Y, w: Integer;
  Line: PLine32;
  Tmp: TFColorA;
...
begin
  ...
  GetMem(Buff, len);
  H := (Bmp.Height - 1) div 2;
  P1 := Bmp.Bits;
  P2 := Bmp.Scanlines[Bmp.Height - 1];
  for I := 0 to H do
  begin
    Move(P1^, Buff^, len);
    Move(P2^, P1^, len);
...
(*
procedure SquareWave8(Src, Dst: TBitmap32; X, Y, z: Double);
var
...
begin
  ...
  GetMem(sy, Dst.Height shl 2);
  for I := 0 to Dst.Width - 1 do
    sx[I] := Round(Sin(I / X) * z);
  for I := 0 to Dst.Height - 1 do
    sy[I] := Round(Sin(I / Y) * z);
  pb := Pointer(Dst.Bits);
  for cy := 0 to Dst.Height - 1 do
  begin
    yy := sy[cy] + cy;
    if (yy >= 0) and (yy < Src.Height) then
    begin
      py := Src.Scanlines[yy];
      for cx := 0 to Dst.Width - 1 do
      begin
        xx := sx[cx] + cx;
        if (xx >= 0) and (xx < Src.Width) then
          pb^ := py[xx];
        inc(pb);
      end;
      inc(pb, Dst.Gap);
...
procedure SquareWave16(Src, Dst: TBitmap32; X, Y, z: Double);
var
  xx, yy, cx, cy, I: Integer;
  sx, sy: PSLut;
  pw: PWord;
...
begin
  ...
  begin
    ...
    begin
      ...
      pw := Ptr(Integer(pw) + Dst.BWidth);
  end;
  FreeMem(sy);
  FreeMem(sx);
end;

procedure SquareWave24(Src, Dst: TBitmap32; X, Y, z: Double);
var
  xx, yy, cx, cy, I: Integer;
  sx, sy: PSLut;
  pc: PFColor;
...
begin
  ...
  begin
    ...
    begin
      ...
      pc := Ptr(Integer(pc) + Dst.BWidth);
  end;
  FreeMem(sy);
  FreeMem(sx);
end;

procedure SquareWave32(Src, Dst: TBitmap32; X, Y, z: Double);
var
  xx, yy, cx, cy, I: Integer;
  sx, sy: PSLut;
  pc: PFColorA;
...
procedure SquareWave(Src, Dst: TBitmap32; X, Y, z: Double);
begin
  case Dst.Bpp of
...
procedure RoundWave8(Src, Dst: TBitmap32; X, Y, z: Double);
var
  xx, yy, cx, cy, I: Integer;
  sx, sy: PSLut;
  pb: PByte;
...
procedure RoundWave16(Src, Dst: TBitmap32; X, Y, z: Double);
var
  xx, yy, cx, cy, I: Integer;
  sx, sy: PSLut;
  pw: PWord;
...
procedure RoundWave24(Src, Dst: TBitmap32; X, Y, z: Double);
var
  xx, yy, cx, cy, I: Integer;
  sx, sy: PSLut;
  pc: PFColor;
...
procedure RoundWave32(Src, Dst: TBitmap32; X, Y, z: Double);
var
  xx, yy, cx, cy, I: Integer;
  sx, sy: PSLut;
  pc: PFColorA;
...
procedure RoundWave(Src, Dst: TBitmap32; X, Y, z: Double);
begin
  case Dst.Bpp of
...
var
  ...
  DstYpad,
    DstXPad: Integer;
begin
  Bottom := Min(Src.Height, ySrc + hSrc);
  Right := Min(Src.Width, xSrc + wSrc);
  Bpp := (Src.Bpp + 7) shr 3;
  LineLen := (Right - xSrc) * Bpp;
  pS := Integer(Src.Scanlines[0]) + (Src.Height - Bottom) * Src.BWidth + xSrc * Bpp;
  DstYpad := Dst.Height + ySrc - Bottom;
  if DstYpad < 0 then
    DstYpad := 0;
...
var
  ...
  Ypad,
    XPad: Integer;
begin
  Bottom := Min(Dst.Height, yDst + Min(Src.Height, hDst));
  Right := Min(Dst.Width, xDst + Min(Src.Width, wDst));
  Bpp := (Src.Bpp + 7) shr 3;
  LineLen := (Right - xDst) * Bpp;
  Ypad := Src.Height + yDst - Bottom;
  if Ypad < 0 then
    Ypad := 0;
  ...
  PD := Integer(Dst.Scanlines[0]) + (Dst.Height - Bottom) * Dst.BWidth + xDst * Bpp;

  for Y := yDst to Bottom - 1 do
  begin
    Move(Ptr(pS)^, Ptr(PD)^, LineLen);
    inc(pS, Src.BWidth);
    inc(PD, Dst.BWidth);
