...
* 单元标识：$Id: CnDebug.pas,v 1.23 2008/05/06 14:56:23 liuxiao Exp $
* 修改记录：2008.05.01
*               增加部分记录入文件的属性。
*           2007.09.24
...
procedure SendDebug(const aMsg: string; const aArg: array of const);overload;
procedure SendDebug(const aMsg: string);overload;
...
procedure SendDebug(const aMsg: string; const aArg: array of const);
begin
	{$IFDEF Debug}
	SendDebug(Format(aMsg, aArg));
	{$ENDIF}
end;
procedure SendDebug(const aMsg: string);
begin
	{$IFDEF Debug}
	CnDebugger.LogMsg(aMsg);
...
begin
  ...
  try
    ...
    if Reg.OpenKey('\Software\CnPack\CnDebug', False) then
      ViewerExe := Reg.ReadString('CnDebugViewer');
  ...
  if 31 < WinExec(PChar(ViewerExe + IntToStr(GetCurrentProcessId)),
