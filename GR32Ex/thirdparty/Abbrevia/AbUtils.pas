...
{!!.04 - Added End }
  ...
  {$IFDEF MSWINDOWS}                                                     {!!.01}
    {$IFDEF COMPILER4_UP} Int64 {$ELSE} LongInt {$ENDIF};                    {!!.01}
  {$ENDIF}                                                               {!!.01}
  {$IFDEF LINUX}                                                         {!!.01}
    Int64;                                                               {!!.01}
  {$ENDIF}                                                               {!!.01}
...
{$IFDEF MSWINDOWS}
function GetLocalDiskFree(const Path : string) : {$IFDEF COMPILER4_UP} Int64 {$ELSE} LongInt {$ENDIF};
var
  SectorsPerCluster,
  BytesPerSector,
  NumberOfFreeClusters, TotalNumberOfClusters : {$IFDEF COMPILER4_UP} Cardinal {$ELSE} LongInt {$ENDIF};
  Succeeded : BOOL;
...
function GetRemoteDiskFree(const Path : string) : {$IFDEF COMPILER4_UP} Int64 {$ELSE} LongInt {$ENDIF};
var
  FreeAvailable, TotalSpace, TotalFree: {$IFDEF COMPILER4_UP} TLargeInteger {$ELSE} LongInt {$ENDIF};
  Succeeded : BOOL;
...
begin
  ...
  begin
    {$IFDEF DefeatWarnings}{$IFDEF COMPILER6_UP} {$WARN SYMBOL_PLATFORM OFF} {$ENDIF} {$ENDIF}
    Result := FileSetDate(f, Age);
    {$IFDEF DefeatWarnings}{$IFDEF COMPILER6_UP} {$WARN SYMBOL_PLATFORM ON} {$ENDIF}{$ENDIF}
    FileClose(f);
  end;
end;
{$ENDIF}
...
{$IFDEF MSWINDOWS}
  {$IFDEF COMPILER4_UP} Int64 {$ELSE} LongInt {$ENDIF};
{$ENDIF}
{$IFDEF LINUX}
  Int64;
{$ENDIF}
