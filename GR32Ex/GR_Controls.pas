...
{ Summary The abstract GR Control Class to support the alpha blending, background, graphics frame and advanced font. }
 ...
 * License.
 *
 * The Original Code is GR_Controls
 *
...
type
  ...
  public
    ...
    { Summary Paint the Component Foreground itself(No parent background) in
      the srcRect. }
    { Description
    绘制该元件的指定范围内的前景（不包括它的父背景）到Buffer (0,0).
    ...
    begin
      ...
      begin
        ...
        //if (vControl is TGRGraphicControl) 
          //or (vControl is TGRCustomControl) then
        begin
          ...
          try
            ...
            Begin
              {$ifdef Debug}
              //GetClipBox(DC, aRect);
              //SendDebug(Format('"Left="%d",Top="%d",Right="%d",Bottom="%d', [aRect.Left, aRect.Top, aRect.Right, aRect.Bottom]));
              {$endif}
...
begin
  ...
  begin
   ...
   try 
    ...
    {else if aControl.Parent is TGRCustomForm then
       with aControl.Parent as TGRCustomForm do
       begin
         P := aControl.ClientOrigin;
         Windows.ScreenToClient(aControl.Parent.Handle, P);
         MoveWindowOrg(DC, -P.X, -P.Y);
...
procedure TGRCustomControl.ClearDefaultPopupMenu(const PopupMenu: TPopupMenu);
begin
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        //对于 子GRCustomControls还是由wINDOWS完成。
        {.$IFDEF FastDraw_Supports}
        if aPaintAll then
...
procedure TGRCustomControl.WMPaint(var Message: TWMPaint);
var
  DC: HDC;
  PS: TPaintStruct;
begin
...
procedure TGRCustomControl.WMSetText(var Message: TMessage);
begin
  inherited;
  {$ifdef debug}
  //SendDebug('Caption Set Text:'+ Pchar(Message.LParam));
  {$endif}
  //if not FSelfBuffer.Empty then FSelfBuffer.Delete;
  //FSelfBufferDirty := True;
  //FBufferDirty := True;
